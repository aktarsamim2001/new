...
import { fetchPrivacyPolicyData } from "@/features/store/privacyPolicySlice";

export default function PrivacyPolicyPage({content}) {

  const page = content?.privacy_policy_page || {};

  return (
    <div className="flex items-center justify-center">
      <div className="w-full container mx-auto __gapTop px-2">
        <h1 className="text-3xl font-bold __secondary-text mb-6">{page?.main_title || "Privacy Policy"}</h1>
        ...
        <div>
          <p className="text-gray-700 mb-4">{page?.short_note}</p>
        </div>
      </div>
    </div>
  );
